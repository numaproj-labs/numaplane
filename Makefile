...
start-e2e: numaflow-crd cleanup-e2e image
	...
	$(KUBECTL) describe -n numaplane-system pod
	$(KUBECTL) describe -n numaplane-system deploy
	$(KUBECTL) wait -n numaplane-system pod --all --for=condition=Ready --timeout=120s
